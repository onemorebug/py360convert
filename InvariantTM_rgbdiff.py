...
def invariantMatchTemplate(rgbimage, rgbtemplate, mask, method, matched_thresh, rgbdiff_thresh, rot_range, rot_interval, scale_range, scale_interval, rm_redundant, minmax):
    """
    ...
    if minmax == False:
        for next_angle in range(rot_range[0], rot_range[1], rot_interval):
            for next_scale in range(scale_range[0], scale_range[1], scale_interval):
                ...
                scaled_mask, _ = scale_image(mask, next_scale, image_maxwh)
                if next_angle == 0:
                    rotated_template = scaled_template_gray
                    rotated_mask = scaled_mask
                else:
                    rotated_template = rotate_image(scaled_template_gray, next_angle)
                    rotated_mask = rotate_image(scaled_mask, next_angle)
                if method == "TM_CCOEFF":
                    ...
                    matched_points = cv2.matchTemplate(img_gray,rotated_template,cv2.TM_CCOEFF_NORMED, mask=mask)
                    satisfied_points = np.where(matched_points >= matched_thresh)
                elif method == "TM_CCORR":
                    matched_points = cv2.matchTemplate(img_gray,rotated_template,cv2.TM_CCORR)
                    satisfied_points = np.where(matched_points >= matched_thresh)
                elif method == "TM_CCORR_NORMED":
                    matched_points = cv2.matchTemplate(img_gray,rotated_template,cv2.TM_CCORR_NORMED, mask=mask)
                    satisfied_points = np.where(matched_points >= matched_thresh)
        ...
        for next_angle in range(rot_range[0], rot_range[1], rot_interval):
            for next_scale in range(scale_range[0], scale_range[1], scale_interval):
                ...
                if method == "TM_CCOEFF":
                    ...
                    print(img_gray.size)
                    print(rotated_template.size)
                    matched_points = cv2.matchTemplate(img_gray,rotated_template,cv2.TM_CCORR_NORMED, mask=rotated_mask)
                    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(matched_points)
                    if max_val >= matched_thresh:
                        all_points.append([max_loc, next_angle, actual_scale, max_val])
                elif method == "TM_SQDIFF":
...
def InvariantTM_rgbdiff(image_path, template_path, mask_path):
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    img_bgr = cv2.imread(image_path)
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
    template_bgr = plt.imread(template_path)
    ...
    plt.show()
    points_list = invariantMatchTemplate(img_rgb, cropped_template_rgb, mask, "TM_CCORR_NORMED", 0.8, 500, [0,360], 10, [100,150], 10, True, True)
